...
func (s *Service) Record(ctx context.Context, trackID, stationID string) error {
...
func (s *Service) BuildQueue(ctx context.Context, stationID string, recentHours int) ([]TrackWithFeatures, error) {
	queue, err := s.queryStationQueue(ctx, stationID, recentHours)
	if err != nil {
		return nil, err
	}
	if len(queue) == 0 {
		queue, err = s.queryStationQueue(ctx, stationID, 0)
		if err != nil {
			return nil, err
		}
	}
...
func (s *Service) queryStationQueue(ctx context.Context, stationID string, recentHours int) ([]TrackWithFeatures, error) {
	recentFilter := ""
	args := []any{stationID}
	if recentHours > 0 {
